...
# Read the creation script to extract the first 5 cells
with open('create_eval_notebook.py', 'r') as f:
    create_lines = f.readlines()

# We'll find the cells.append lines for the first 5 cells.
# We know the pattern: cells.append(nbf.v4.new_(markdown|code)_cell(""" ... """))
# We'll extract the content between the triple quotes for the first 5 matches.
pattern = r'cells\.append\(nbf\.v4\.new_(markdown|code)_cell\("""([\s\S]*?)"""\)'
matches = re.findall(pattern, ''.join(create_lines), re.DOTALL)
# matches is a list of tuples (cell_type, content)
...
# Convert the matches to nbformat cells
for cell_type, content in first5:
    # The content string may have a trailing newline? We'll split by newline and ensure each line ends with newline.
    lines = content.split('\n')
    # If the last line is empty, drop it
    if lines and lines[-1] == '':
        lines.pop()
    # Now, we want each line to end with a newline character
    lines = [line + '\n' for line in lines]
...
# Now we have the first 5 cells. We need to add the remaining cells (6 to 11).
# We'll define the remaining cells as lists of lines.
...
cells.append(nbf.v4.new_code_cell(''.join(cell6_lines)))
...
cells.append(nbf.v4.new_code_cell(''.join(cell7_lines)))
...
cell8_lines = [
    ...
    "print(f\"Total explained variance (32 components)
