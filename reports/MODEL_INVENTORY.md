...
- **Production Readiness:** 
  ...
  - 🔓 Unblocking conditions:
                1. Publicly distributable EEGPT checkpoint with clear license (BSD-3/MIT/Apache-2.0 preferred)
                2. Verified ONNX export path (PyTorch→ORT cosine >0.999)
                3. Runtime decision: Server-side (recommended) or quantized WebGPU ONNX
- **Evidence:** 
...
- **Production Readiness:** 
  ...
  - 🔓 Unblocking conditions:
                1. Labeled dataset with EEG and cognitive state annotations
                2. Training pipeline for lightweight model (logistic regression → shallow NN)
                3. ONNX/TF.js export for browser deployment
- **Evidence:** 
