...
function reluG(x: number) {
  return x > 0 ? 1 : 0;
}
function sig(x: number) {
  return 1 / (1 + Math.exp(-Math.max(-20, Math.min(20, x))));
}
function sigG(y: number) {
  return y * (1 - y);
}
function fwd(
  x: number[],
  ...
  const z1 = W1.map((r, i) => r.reduce((s, w, j) => s + w * x[j], B1[i]));
  const h1 = z1.map(relu);
  const z2 = W2.map((r, i) => r.reduce((s, w, j) => s + w * h1[j], B2[i]));
  const h2 = z2.map(relu);
  const z3 = W3.map((r, i) => r.reduce((s, w, j) => s + w * h2[j], B3[i]));
  const out = z3.map(sig);
  return { h1, h2, out };
}
...
function trainMLP(
  ...
  h1 = 32,
  h2 = 16,
  onEpoch?: (e: number, l: number, v: number) => void,
): TrainedWeights {
  const I = 5,
    O = 3;
  let W1 = initW(h1, I, Math.sqrt(2 / I)),
    B1 = new Array(h1).fill(0);
  let W2 = initW(h2, h1, Math.sqrt(2 / h1)),
    B2 = new Array(h2).fill(0);
  let W3 = initW(O, h2, Math.sqrt(2 / h2)),
    B3 = new Array(O).fill(0);
  const b1 = 0.9,
    b2 = 0.999,
    eps = 1e-8;
  let t = 0;
  ...
  const tr = sh.slice(0, split),
    va = sh.slice(split);
  function adam(
    p: number[][],
    g: number[][],
    ms: ReturnType<typeof mW>,
    vs: ReturnType<typeof mW>,
    t: number,
  ) {
  ...
  function adamV(
    p: number[],
    g: number[],
    ms: ReturnType<typeof mV>,
    vs: ReturnType<typeof mV>,
    t: number,
  ) {
    const lr_t = (lr * Math.sqrt(1 - b2 ** t)) / (1 - b1 ** t);
    return p.map((v, i) => {
      ms.m[i] = b1 * ms.m[i] + (1 - b1) * g[i];
      vs.v[i] = b2 * vs.v[i] + (1 - b2) * g[i] ** 2;
      return v - (lr_t * ms.m[i]) / (Math.sqrt(vs.v[i]) + eps);
    });
  }
  let bestVal = Infinity,
    best = { W1, B1, W2, B2, W3, B3 },
    pat = 0;
  for (let ep = 0; ep < epochs; ep++) {
    ...
    for (const s of batch) {
      ...
      for (let i = 0; i < O; i++) {
        for (let j = 0; j < h2; j++) dW3[i][j] += dZ3[i] * a2[j];
        dB3[i] += dZ3[i];
      }
      ...
      for (let i = 0; i < h2; i++) {
        for (let j = 0; j < h1; j++) dW2[i][j] += dZ2[i] * a1[j];
        dB2[i] += dZ2[i];
      }
      ...
      for (let i = 0; i < h1; i++) {
        for (let j = 0; j < I; j++) dW1[i][j] += dZ1[i] * s.features[j];
        dB1[i] += dZ1[i];
      }
    }
    ...
    const vl =
      va.reduce((s, sv) => {
        const { out } = fwd(sv.features, W1, B1, W2, B2, W3, B3);
        return s + out.reduce((s2, o, i) => s2 + (o - sv.labels[i]) ** 2, 0) / O;
      }, 0) / Math.max(1, va.length);
  ...
  return {
    ...
    meta: {
      ...
      n_subjects: new Set(samples.map((s) => s.subject)).size,
      val_mse: bestVal,
      epochs_trained: t,
...
export async function runTrainingPipeline(
    ...
    for (const record of recs) {
      ...
      try {
        ...
        rep({
          ...
          subject: record.id,
          samplesLoaded: samples.length,
          message: `Preprocessing ${record.id}…`,
        });
        const { windows } = preprocess(signal, {
          bandpass: { low: 1, high: 40 },
          notch: { fc: 60 },
          normalize: true,
          segment: { windowSec: 2, overlap: 0.5 },
          artifactRejection: { enabled: true },
        });
        ...
        for (let aug = 0; aug < 5; aug++) {
          ...
          samples.push({
            features: noisy.map((v) => v / total),
            labels: labels.map((l) => Math.min(1, Math.max(0, l + (Math.random() - 0.5) * 0.05))),
            subject,
            task,
          });
        }
        ...
        rep({
          phase: "fetching",
          subject: record.id,
          message: `Skipped ${record.id}: ${(err as Error).message}`,
        });
      }
    }
  }
  if (samples.length < 20)
    throw new Error(`Not enough samples (got ${samples.length}). Check PhysioNet access.`);
  rep({
    phase: "training",
    samplesLoaded: samples.length,
  ...
  rep({
    phase: "done",
    samplesLoaded: samples.length,
    message: `Done! val_mse=${weights.meta.val_mse.toFixed(4)}`,
  });
  return weights;
}
