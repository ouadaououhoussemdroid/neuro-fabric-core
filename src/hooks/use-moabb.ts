...
export const MOABB_DATASETS: MOABBDataset[] = [
  {
    ...
    description: "Motor imagery: left hand, right hand, feet, tongue. 9 subjects × 2 sessions.",
    n_subjects: 9,
    n_sessions: 2,
    n_runs: 6,
    ...
    sfreq: 250,
    n_channels: 22,
    scientific_ref: "Tangermann et al. 2012",
  },
  {
    id: "BNCI2014_004",
    name: "BCI Competition IV 2b",
    description: "Motor imagery: left hand, right hand. 9 subjects × 5 sessions.",
    n_subjects: 9,
    n_sessions: 5,
    n_runs: 5,
    paradigm: "MotorImagery",
    sfreq: 250,
    n_channels: 3,
    scientific_ref: "Leeb et al. 2008",
  },
  {
    id: "Cho2017",
    name: "Cho 2017",
    description: "Motor imagery: left hand, right hand. 52 subjects.",
    n_subjects: 52,
    n_sessions: 1,
    n_runs: 1,
    paradigm: "MotorImagery",
    sfreq: 512,
    n_channels: 64,
    scientific_ref: "Cho et al. 2017",
  },
  {
    id: "PhysionetMI",
    name: "PhysioNet Motor Imagery",
    description: "Motor imagery: hands and feet. 109 subjects.",
    n_subjects: 109,
    n_sessions: 1,
    n_runs: 14,
    paradigm: "MotorImagery",
    sfreq: 160,
    n_channels: 64,
    scientific_ref: "Goldberger et al. 2000",
  },
];

export function useMOABB() {
  const [progress, setProgress] = useState<MOABBProgress>({
    phase: "idle",
    message: "Not started",
    progress: 0,
  });
...
json.dumps({"epochs": epochs_out, "n_total": X.shape[0], "n_channels": X.shape[1], "n_times": X.shape[2], "labels": list(set(labels.tolist()))})
      ...
      set({
        phase: "done",
        message: `Loaded ${parsed.n_total} epochs from ${datasetId}`,
        progress: 100,
      });
      return parsed;
    } catch (err) {
