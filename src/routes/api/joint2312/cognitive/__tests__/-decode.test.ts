/**
 * M33 — API route tests for POST /api/joint2312/cognitive/decode.
 *
 * Mirrors the M32 subject-identity route tests: validates auth, rate limiting,
...
// ─── Mock ONNX adapter (for transitive imports) ────────────────────────────

const { mockPredict, mockBuildServiceProvenance, mockAuthenticateRequest, mockCheckRateLimit, mockDecodeCognitiveState } =
  vi.hoisted(() => ({
    mockPredict: vi.fn(),
    mockBuildServiceProvenance: vi.fn(),
    mockAuthenticateRequest: vi.fn(),
    mockCheckRateLimit: vi.fn(),
    mockDecodeCognitiveState: vi.fn(),
  }));
...
// Mock the cognitive server to isolate route-layer behavior
class MockCognitiveDecodeError extends Error {
...
vi.mock("@/lib/ai/inference/cognitive.server", () => ({
  ...
  CognitiveDecodeError: MockCognitiveDecodeError,
  COGNITIVE_TIMEOUT_MS: 10_000,
}));

describe("POST /api/joint2312/cognitive/decode route", () => {
  ...
  beforeEach(() => {
    ...
    mockDecodeCognitiveState.mockResolvedValue({
      ...
      timings: { inference_ms: 0.42, total_ms: 1.2 },
    });
  });
  ...
  it("returns 200 with decode results for valid request", async () => {
    ...
    const body = await result.json();
    expect(body.service).toBe("cognitive-intelligence");
    expect(body.head).toBe("cognitive-linear-v1");
    expect(body.results[0].metric).toBe("workload");
  });

  it("returns 401 on AuthError", async () => {
    mockAuthenticateRequest.mockReset();
    mockAuthenticateRequest.mockRejectedValue(
      new MockCognitiveDecodeError("Unauthorized", "AUTH"),
    );
    // Use the real AuthError class from the mocked module
  ...
  it("returns 400 on CognitiveDecodeError", async () => {
    // The route catches CognitiveDecodeError and returns 400 with code
    mockDecodeCognitiveState.mockRejectedValue(
      new MockCognitiveDecodeError("embedding_id not found or access denied", "EMBEDDING_NOT_FOUND"),
    );
    mockRequest.json = vi.fn().mockResolvedValue({
  ...
  it("returns 500 on unknown error", async () => {
    mockDecodeCognitiveState.mockRejectedValue(new Error("Unexpected failure"));
    mockRequest.json = vi.fn().mockResolvedValue({
      query_embedding: new Array(2312).fill(0.1),
      query_type: "workload",
    });
    ...
    const body = await result.json();
    expect(body.error).toBe("An error occurred during cognitive decoding.");
  });
  ...
  it("returns 408 on timeout", async () => {
    ...
    // Make decode hang indefinitely — the route's internal 10s timeout
    // will reject first, returning 408.
    mockDecodeCognitiveState.mockImplementation(() => new Promise(() => {}));
    ...
    // Advance past the timeout (10000ms + 1ms buffer)
    vi.advanceTimersByTime(10001);

    // Need to flush microtasks for the promise to settle
