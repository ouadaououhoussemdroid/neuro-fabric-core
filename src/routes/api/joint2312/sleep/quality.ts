/**
 * M40 — Sleep Quality decode endpoint.
 *
 * Route: POST /api/joint2312/sleep/quality
 *
 * Decodes sleep quality (normalized [0, 1]) from a Joint-2312 embedding
 * using a linear regression probe (2312→1).
 *
 ...
 * used directly.
 *
 * Security: same auth/rate-limit/CORS pattern as `/api/joint2312/sleep/decode`.
 */
...
import {
  ...
  type SleepQualityDecodeResponse,
  SleepDecodeError,
  SLEEP_TIMEOUT_MS,
...
export const Route = createFileRoute("/api/joint2312/sleep/quality")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const timeoutPromise = new Promise<never>((_, reject) => {
          timeoutId = setTimeout(() => {
            reject(new Error(`Sleep quality decode timeout (${PROCESSING_TIMEOUT_MS}ms)`));
        ...
        try {
          ...
          if (msg.includes("timeout")) {
            log("error", "sleep.quality.timeout_exceeded", { timeoutMs: PROCESSING_TIMEOUT_MS });
            return res(requestOrigin, { error: "Processing timeout. Please try a smaller file." }, { status: 408 });
          }
          log("error", "sleep.quality.failed", { error: msg });
          metrics.sleepDecodeErrorsTotal.inc({ error: "decode_failed" });
          metrics.tier1ServiceErrorsTotal.inc({ service: "sleep-staging" });
          return res(requestOrigin, { error: "An error occurred during sleep quality inference." }, { status: 500 });
        } finally {
        ...
        async function processDecode(): Promise<Response> {
          ...
          try {
            ...
            log("error", "sleep.quality.auth_unexpected", {
          ...
          try {
            ...
            log("error", "sleep.quality.rate_limit_check_failed", {
          ...
          const req = body as SleepQualityDecodeRequest;

          // Validate query_type
          const validQueryTypes = ["sleep-quality"];
          if (!req.query_type || !validQueryTypes.includes(req.query_type)) {
            return res(
              requestOrigin,
              { error: `query_type must be "sleep-quality" (or omit for default)` },
              { status: 400 },
            );
          }
          ...
          let response: SleepQualityDecodeResponse;
          try {
            response = await decodeSleepQuality(req, supabase, userId);
          } catch (decodeErr) {
            if (decodeErr instanceof SleepDecodeError) {
              log("warn", "sleep.quality.decode_error", {
                code: decodeErr.code,
