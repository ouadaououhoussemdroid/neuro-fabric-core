...
function UploadPage() {
  ...
  return (
    <DashboardShell fullName={name} role={role}>
      ...
      <p className="mt-2 text-sm text-muted-foreground">Supported: EDF, BDF, CSV, TSV, NPY — max 10 MB</p>
      ...
      <div className="mt-8 flex flex-col gap-4 max-w-2xl">
        <GlassCard>
          <label className="flex flex-col items-center gap-3 cursor-pointer py-6">
            ...
            <span className="text-sm text-muted-foreground">{file ? file.name : "Tap to choose a file"}</span>
            <input type="file" accept=".edf,.bdf,.csv,.tsv,.npy" className="hidden" onChange={(e) => handleFileChange(e.target.files?.[0] ?? null)} />
          </label>
        </GlassCard>

        {file && (file.name.toLowerCase().endsWith(".csv") || file.name.toLowerCase().endsWith(".npy") || file.name.toLowerCase().endsWith(".tsv")) && (
          <GlassCard>
            <label className="text-xs text-muted-foreground">Sample Rate (Hz)</label>
            <input type="number" value={sampleRate} onChange={(e) => setSampleRate(e.target.value)} className="mt-1 w-full rounded border border-border bg-background px-3 py-2 text-sm" placeholder="e.g. 256" />
          </GlassCard>
        )}
        ...
        <button onClick={handleUpload} disabled={!file || loading || quality?.overall === "bad"} className="rounded-lg bg-neuro px-6 py-3 text-sm font-semibold text-white disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-90 transition-opacity">
          {loading ? "Processing…" : quality?.overall === "bad" ? "Fix signal quality issues first" : "Upload & Analyse"}
        </button>
...
function QualityCard({ report }: { report: SignalQualityReport }) {
  const Icon = report.overall === "good" ? CheckCircle : report.overall === "warning" ? AlertTriangle : XCircle;
    ...
    <GlassCard>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold" style={{ color }}>Signal Quality: {qualityLabel(report.overall)}</span>
        </div>
        <span className="font-mono text-sm font-bold" style={{ color }}>{report.score}/100</span>
      </div>
      <div className="mt-3 h-2 rounded-full bg-muted/60">
        <div className="h-full rounded-full transition-all" style={{ width: `${report.score}%`, backgroundColor: color }} />
      </div>
      {report.warnings.map((w, i) => (
        <p key={i} className="mt-2 text-xs text-amber-400 flex items-center gap-1"><AlertTriangle className="h-3 w-3 shrink-0" /> {w}</p>
      ))}
      {report.errors.map((e, i) => (
        <p key={i} className="mt-1 text-xs text-destructive flex items-center gap-1"><XCircle className="h-3 w-3 shrink-0" /> {e}</p>
      ))}
            ...
            {report.channels.map((ch) => (
              <div key={ch.channel} className="flex items-start justify-between text-xs">
                ...
                <div className="flex-1 flex flex-col gap-0.5">
                  {ch.issues.length === 0 ? <span className="text-green-500">No issues</span> : ch.issues.map((issue, i) => <span key={i} style={{ color: qualityColor(ch.level) }}>{issue}</span>)}
                </div>
...
function ResultCard({ result }: { result: Record<string, unknown> }) {
  ...
  return (
    <GlassCard className="border-neuro/30">
      ...
      {signal && <p className="text-xs text-muted-foreground">{String(signal.channels)} · {String(signal.sampleRate)} Hz</p>}
