...
const SAMPLES = [
  { label: "subj_0421 · visual.faces", caption: "a young woman with red hair, frontal portrait", conf: 0.78, align: 0.84, hue: 12 },
  { label: "subj_0118 · visual.scenes", caption: "a snow-covered mountain range at dusk", conf: 0.71, align: 0.79, hue: 220 },
  { label: "subj_0712 · visual.animals", caption: "a golden retriever sitting on green grass", conf: 0.74, align: 0.81, hue: 70 },
];
...
function EEG2ImagePage() {
  ...
  const trace = Array.from({ length: 120 }, (_, i) => ({ t: i, v: Math.sin(i / 4) * 0.6 + Math.sin(i / 1.8) * 0.3 + (Math.random() - 0.5) * 0.2 }));

  const run = () => {
    setRunning(true);
    setOut(null);
    setTimeout(() => { setOut(SAMPLES[idx]); setRunning(false); }, 1100);
  };
    ...
    <SiteShell>
      <Section>
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
          <GlassCard>
            <div className="flex flex-wrap items-center gap-2">
              {SAMPLES.map((s, i) => (
                <button
                  key={i}
                  onClick={() => { setIdx(i); setOut(null); }}
                  className={`rounded-md border px-3 py-1.5 font-mono text-[11px] uppercase tracking-wider ${
                    idx === i ? "border-neuro/60 bg-neuro/10" : "border-border text-muted-foreground hover:text-foreground"
                  }`}
                >
                  {s.label}
                </button>
            ...
            <div className="mt-4 h-40">
              <ResponsiveContainer>
                <LineChart data={trace}>
                  <Line type="monotone" dataKey="v" stroke="oklch(0.85 0.18 195)" strokeWidth={1.1} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
            <button
          ...
          <GlassCard>
            <div className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Reconstructed output</div>
            <div className="mt-3 relative aspect-square overflow-hidden rounded-lg border border-border">
              ...
              />
              {!out && !running && (
                <div className="relative grid h-full place-items-center text-xs text-muted-foreground">no output yet</div>
              )}
              {running && (
...
function Metric({ label, value }: { label: string; value: number }) {
  return (
    <div>
      <div className="flex justify-between text-xs"><span className="text-muted-foreground">{label}</span><span className="font-mono text-neuro">{value.toFixed(2)}</span></div>
