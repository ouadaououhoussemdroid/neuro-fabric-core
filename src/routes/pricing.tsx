...
const TIERS = [
  {
    name: "Research", price: "Free", per: "for accredited labs",
    desc: "For academic teams running non-commercial experiments.",
    features: ["10k embeddings / mo", "1k reconstructions / mo", "Public synthetic datasets", "Community Slack"],
    cta: "Apply for credits", highlight: false,
  },
  {
    name: "Startup", price: "$499", per: "/ month",
    desc: "Build BCI products with production-grade SLAs.",
    features: ["1M embeddings / mo", "50k reconstructions / mo", "Custom fine-tunes", "SOC 2 reports", "Email support"],
  ...
  },
  {
    ...
    features: ["Unlimited inference", "Dedicated regions", "VPC peering · CMEK", "HIPAA BAA", "Solutions engineering"],
    cta: "Talk to sales", highlight: false,
  },
  {
    name: "Sovereign AI", price: "Contact", per: "on-prem · air-gapped",
    desc: "For governments and defense research orgs.",
    features: ["Self-hosted runtime", "Weights licensing", "FedRAMP High roadmap", "24/7 on-call", "Dedicated SE team"],
    cta: "Request brief", highlight: false,
  },
];
    ...
    <SiteShell>
      <Section>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {TIERS.map((t) => (
            <GlassCard key={t.name} className={`flex h-full flex-col ${t.highlight ? "border-neuro/40 ring-glow" : ""}`}>
              <div className="flex items-center justify-between">
                <div className="text-sm font-semibold">{t.name}</div>
                {t.highlight && <span className="rounded-full bg-neuro/15 px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-neuro">popular</span>}
              </div>
              ...
              <ul className="mt-5 space-y-2 text-sm">
                {t.features.map((f) => (
                  <li key={f} className="flex items-start gap-2"><Check className="mt-0.5 h-4 w-4 text-neuro" /><span className="text-muted-foreground">{f}</span></li>
                ))}
              </ul>
              <Link
                to="/about"
                className={`mt-6 inline-flex items-center justify-center rounded-md px-3 py-2 text-xs font-medium ${
                  t.highlight ? "bg-neuro-gradient text-background glow" : "border border-border bg-card/40 hover:bg-card"
                }`}
              >
                {t.cta}
              </Link>
