...
cells.append(nbf.v4.new_code_cell("""# Load the ONNX model
...
# We expect the input to be [batch, channels, samples] or [batch, 1, channels, samples]?
# Let's check the model's input shape from the ONNX file.
# We'll run a dummy input to see the expected shape.
...
# Run the model to see the output shape
try:
    ...
    print(f"Error running dummy input: {e}")
    # Try with an extra dimension if needed
    dummy_input2 = np.random.randn(1, 1, config["signal"]["channels"], config["signal"]["window_samples"]).astype(np.float32)
    dummy_output = ort_session.run(None, {input_name: dummy_input2})
    print(f"Dummy output shapes with extra dim: {[out.shape for out in dummy_output]}")
    # If this works, we need to adjust the input shape
    if len(dummy_input2.shape) == 4:
        print("Model expects input shape [batch, 1, channels, samples]")
        # We'll adjust our input accordingly later
"""))
...
cells.append(nbf.v4.new_code_cell("""def run_eegconformer_onnx(X_batch):
    ...
    """
    # Determine if we need to add a channel dimension (for the unsqueeze in the model)
    # We'll check the input shape from the session.
    input_shape = ort_session.get_inputs()[0].shape
    if len(input_shape) == 4 and input_s
