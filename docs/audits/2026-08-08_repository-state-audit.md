...
However, the headline claim — _"EEGConformer live in default routing"_ — is **false**:
...
1. **Dimension mismatch (silent ANN write failure):** `upload.ts` computes a 64-dim PCA
   ...
   and logs as `eeg.upload.vector_store_failed` (best-effort) — so the upload _succeeds_
...
inherits the **Jul-24 baseline**, _not_ the Aug-8 audited state.
...
the exact answer to _"what do I inherit?"_.
...
- _Public marketing/concept:_ `index`, `about`, `architecture`, `developers`, `eeg2image`,
  `embeddings`, `playground`, `pricing`, `research`, `signin`, `signup`, `studio`,
  `synthetic`.
- _Authenticated product:_ `_authenticated/dashboard.*` (`analyses`, `datasets`,
  `experiments`, `models`, `onnx`, `training`, `upload`), `_authenticated/route.tsx`,
- _API:_ `api/eeg/upload`, `api/evaluate/cross-subject`, `api/health`,
...
states: _"this page renders locally-generated demo points, not live production
embeddings."_ `src/routes/eeg2image.tsx:76` states: _"it is not connected to a live
image-generation model."_ Commit `48ad200` ("fix: stop presenting fabricated metrics and
...
- `tier4-final-gate.test.ts` **VERIFICATION 1** — drives the **public `embedEEG()`** entry
  point the same way (each model registered under a _temp id_ with a pre-built real
...
- `onnx-artefact-integration.test.ts` — **does NOT** run inference. It verifies file
  ...
  build a Node session (the test literally comments: _"We can't run forward inference in
  Node without a WASM backend"_). So this file is **integrity + metadata**, not inference.
...
_the production EEGConformer factory path is not exercised end-to-end with a real runtime._
...
So the _mechanism_ for Phase 4.2/4.3 (rollout gate + per-user cohort routing) is implemented
...
(see §5.4). So Phase 4.2/4.3's _control plane_ is real; its _data plane_ (being called from a
...
2. **Dimension-mismatch data loss in the ANN index.** `upload.ts` writes a 64-dim PCA vector
   ...
   `eeg.upload.vector_store_failed` as a _warning_, so the upload succeeds but **the vector
...
- The EEG preprocessing + embedding (`embedSignal`, PCA/AE, 64-dim), the cognitive decoder
  (`decodeCognitiveState` → ONNX logistic regression with heuristic fallback — this _is_
...
_Audit authored 2026-08-08 against the live working tree. All command outputs cited
...
untracked models) were produced during this audit and are reproducible from the repo root._
