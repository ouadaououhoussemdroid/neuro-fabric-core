...
Difficulty is _technical risk_ (unknown unknowns). Effort is _wall-clock
work_ assuming the dependencies exist. They are independent: a task can be
...
Qdrant, DVC, EEGPT, reconstruction — is _optionality_, not _vision
completion_. The original Neuro-Fabric vision (acquire → embed →
