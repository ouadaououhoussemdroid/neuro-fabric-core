...
n_ch = len(ch_names); n_patch_v = 8; psize = 200
torch.manual_seed(42)
eeg = torch.randn(1, n_ch, n_patch_v, psize)
model.eval()
...
class LaBraMWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + pe
        te = self.time_embed[:, 0:itw, :].unsqueeze(1).expand(b, self.n_channels, -1, -1).flatten(1, 2)
        x[:, 1:, :] += te
        x = self.pos_drop(x)
...
wrapper = LaBraMWrapper(model, input_chans, n_ch, n_patch_v, psize)
wrapper.eval()
with torch.no_grad():
    wout = wrapper(eeg)
...
print(f'Batch=1: max_diff={mdiff:.6e}, all_close(1e-3)={np.allclose(oout, pout, atol=1e-3)}')

all_pass = True
for bs in [2, 4, 8, 16]:
    eeg_b = torch.randn(bs, n_ch, n_patch_v, psize)
    with torch.no_grad(): pout_b = wrapper(eeg_b).numpy()
    oout_b = sess.run(None, {iname: eeg_b.numpy()})[0]
    mdiff_b = np.abs(oout_b - pout_b).max()
    ok = np.allclose(oout_b, pout_b, atol=1e-3)
    if not ok: all_pass = False
    print(f'Batch={bs}: max_diff={mdiff_b:.6e}, all_close(1e-3)={ok}')
...
print(f'\n{"="*70}')
print(f'LaBraM VERIFICATION COMPLETE'
